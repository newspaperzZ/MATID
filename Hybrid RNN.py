...
# 自定义训练集
df_for_training=df[0:2694]
df_for_testing=df[2694:3027]
...
time_step=5  #历史多少数据预测
trainX,trainY=createXY(df_for_training_scaled,time_step)
testX,testY=createXY(df_for_testing_scaled,time_step)

sgd = optimizers.SGD(lr=0.003, decay=1e-6, momentum=0.9, nesterov=True)#定义一个优化器

def build_model(optimizer):
    grid_model = Sequential()
    grid_model.add(LSTM(128,return_sequences=True,input_shape=(time_step,end-start-1)))
    grid_model.add(LSTM(64,return_sequences=True))
    grid_model.add(LSTM(32,return_sequences=True))
    grid_model.add(GRU(16,return_sequences=True))
    grid_model.add(GRU(8,return_sequences=True))
    grid_model.add(GRU(4,dropout=0.2))
...
history=my_model.fit(trainX,trainY,validation_data=(testX, testY),epochs=40,batch_size=5,verbose=2)
# grid_model = KerasRegressor(build_fn=build_model,verbose=2)
#
#
# parameters = {'batch_size' : [80],
#               'epochs' : [200],
#               'optimizer' : [sgd] }#,'Adadelta'动态生成学习率'adam','Adadelta'
#
